...
Le package msa permet d'utiliser les algorithmes classiques d'alignement de multiples séquences. Ouvrir la page d'aide de la fonction msa.
...
Comment évolue les virus de la grippe ? Pourquioi évoluent-ils particulièrement rapidement ?

# Méthode de distances

Rappeler le principe des méthodes de distance, en citer deux exemples. Quelles sont les différences entre ces méthodes ?

## Génération de la matrice de distances

Nous allons générer une matrice des distances à partir de nos alignements. aller sur [ce lien](http://www.iqtree.org/doc/Substitution-Models) et [celui-ci](https://en.wikipedia.org/wiki/Models_of_DNA_evolution) et regarder les différents modèles. Quelles sont les différences générales entre ces modèles ? Nous allons utiliser le modèle TN93, pourquoi ce nom ? Quelles sont ses particularités ?
...
Commenter cet arbre \## Génération de l'arbre par Neighbour-Joining Ouvrir l'aide de la fonction nj(). Générer l'arbre et le visualiser avec la fonction plot().
...
Nous allons raciner l'arbre de manière arbitraire. Pour commencer, explorer la structure de l'objet NJ_tree. Utiliser les fonctions plot() et nodelabel() pour afficher l'arbre et les numéros des noeuds.
...
Commenter cet arbre. \## Quelle méthode choisir ? Nous allons voir comment
