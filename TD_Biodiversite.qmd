...
Quel est le protocole général pour construire un arbre phylogénétique à partir de séquences ADN ? (5 min)

# Acquisition des séquences

Nous allons commencer par récupérer les séquences d’intérêts sur Genbank directement depuis `R`.\
Qu'est-ce que [GenBank](https://www.ncbi.nlm.nih.gov/genbank/) ? (5 min)
...
Aller sur [GenBank](https://www.ncbi.nlm.nih.gov/nucleotide/) et entrer un numéro d'accès dans la barre de recherche.
...
Pourqu'oi s'intéresser à cette séquence en particulier ? (10 min)
...
Nous allons visualiser la matrice de distance avec `ggplot2`, un des nombreux packages composant le [Tidyverse](https://www.tidyverse.org/). Décrire la structure de cette portion de code. (5 min)
